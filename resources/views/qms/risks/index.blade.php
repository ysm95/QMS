...
<section class="view active-view">
  <div class="page-title"><div><p class="eyebrow">Safety risk management</p><h1>Risk register</h1></div><span class="status-pill warning">5 x 5 register</span></div>
  ...
  <div class="table-panel"><table><thead><tr><th>Reference</th><th>Hazard</th><th>Owner</th><th>Rating</th><th>Controls</th><th>Review</th></tr></thead><tbody>
    @foreach ($risks as $risk)
      <tr><td>{{ $risk->reference }}</td><td>{{ $risk->hazard }}</td><td>{{ $risk->owner }}</td><td><span class="risk-badge">{{ $risk->rating }}</span></td><td>{{ $risk->controls }}</td><td>{{ optional($risk->review_date)->format('Y-m-d') }}</td></tr>
    @endforeach
  </tbody></table></div>
