...
<section class="view active-view">
  <div class="page-title"><div><p class="eyebrow">Controlled information</p><h1>Documents</h1></div><span class="status-pill success">Document control</span></div>
  ...
  <div class="table-panel"><table><thead><tr><th>Reference</th><th>Title</th><th>Version</th><th>Owner</th><th>Status</th><th>Review</th></tr></thead><tbody>
    @foreach ($documents as $document)
      <tr><td>{{ $document->reference }}</td><td>{{ $document->title }}</td><td>{{ $document->version }}</td><td>{{ $document->owner }}</td><td><span class="status-pill">{{ $document->status }}</span></td><td>{{ optional($document->review_date)->format('Y-m-d') }}</td></tr>
    @endforeach
  </tbody></table></div>
